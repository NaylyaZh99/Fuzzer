...
def random_strategy(model,model_layer_times, neuron_to_cover_num):
    ...
    for _ in range(neuron_to_cover_num):
        ...
        loss00_neuron = tf.math.reduce_mean(model.get_layer(layer_name).output[..., index])
        print("random_strategy: ", loss00_neuron)
        # if loss_neuron == 0:
...
def neuron_selection(model, model_layer_times, model_layer_value, neuron_select_strategy, neuron_to_cover_num,threshold, img):
    ...
    if '0' in list(neuron_select_strategy):
        ...
        for num in num_neuron0:
            ...
            # loss0_neuron = tf.math.reduce_mean(model.get_layer(layer_name0).output[..., index0])
            conv_neuron_layer = model.get_layer(layer_name0)
            heatmap_model = models.Model([model.inputs], [conv_neuron_layer.output])
            loss0_neuron = tf.math.reduce_mean(heatmap_model(img)[..., index0])
    ...
    if '1' in list(neuron_select_strategy):
        ...
        for num in num_neuron1:
            ...
            # loss1_neuron = tf.math.reduce_mean(model.get_layer(layer_name1).output[..., index1])
            conv_neuron_layer = model.get_layer(layer_name1)
            heatmap_model = models.Model([model.inputs], [conv_neuron_layer.output])
            loss1_neuron = tf.math.reduce_mean(heatmap_model(img)[..., index1])
    ...
    if '2' in list(neuron_select_strategy):
        ...
        for i in num_neuron2:
            ...
            # print('layer_name2:', layer_name2)
            conv_neuron_layer = model.get_layer(layer_name2)
            index2 = model_layer_weights_top_k[i][1]
            heatmap_model = models.Model([model.inputs], [conv_neuron_layer.output])
            loss2_neuron = tf.math.reduce_mean(heatmap_model(img)[..., index2])
            # loss2_neuron = tf.math.reduce_mean(model.get_layer(layer_name2).output[..., index2])
            # print("loss2_neuron:", loss2_neuron)
    ...
    if '3' in list(neuron_select_strategy):
        ...
        if len(above_threshold) > 0:
            for above_item in range(len(above_threshold)):
                # loss_neuron.append(tf.math.reduce_mean(
                #     model.get_layer(above_threshold[above_item][0]).output[..., above_threshold[above_item][1]]))
                conv_neuron_layer = model.get_layer(above_threshold[above_item][0])
                heatmap_model = models.Model([model.inputs], [conv_neuron_layer.output])
                loss3_neuron = tf.math.reduce_mean(heatmap_model(img)[..., above_threshold[above_item][1]])
                loss_neuron.append(loss3_neuron)
        ...
        if len(below_threshold) > 0:
            for below_item in range(len(below_threshold)):
                ...
                conv_neuron_layer = model.get_layer(below_threshold[below_item][0])
                heatmap_model = models.Model([model.inputs], [conv_neuron_layer.output])
                loss2_neuron = tf.math.reduce_mean(heatmap_model(img)[..., below_threshold[below_item][1]])
                loss_neuron.append(loss3_neuron)
...
def neuron_scale(loss_neuron):
    loss_neuron_new = []
    loss_sum = tf.math.reduce_sum(loss_neuron)
...
def neuron_scale_maxmin(loss_neuron):
    max_loss = tf.math.reduce_max(loss_neuron)
    min_loss = tf.math.reduce_min(loss_neuron)
